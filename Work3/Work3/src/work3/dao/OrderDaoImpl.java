...
public class OrderDaoImpl implements OrderDao {
    ...
    public ArrayList<Order> selectAll(String orderBy) {
        ...
        try {
            String sql = String.format("select %s from `order` order by %s", columns, orderBy);
            ...
            throw new DataNotFoundException("未查询到订单, 请校验查询条件合法性");
        } finally {
    ...
    public Order selectById(int orderId) {
        try {
            String sql = String.format("select %s from `order` where %s = ?", columns, id);
            rs = JdbcUtils.select(conn, stmt, sql, orderId);
            rs.next();
            String dateString = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(rs.getTimestamp(createTime));
    ...
    public int update(Order order) {
        try {
            String sql = String.format("update `order` set %s = %d, %s = %f where %s = ?", count, order.getCount(), price, order.getOrderPrice(), id);
    ...
    public int insert(Order order) {
        try {
            ...
            return JdbcUtils.update(conn, stmt, sql, order.getGoodId(), order.getCount(), order.getOrderPrice());
        } catch (Exception e) {
